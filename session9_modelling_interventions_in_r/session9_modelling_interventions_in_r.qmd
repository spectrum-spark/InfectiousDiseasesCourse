...
description: "Take the model of COVID and introduce interventions"
format:
    training-html:
        presenter: <br>Dewi Anastasia Christina <br>OUCRU Indonesia
        contributors:
            ...
            - Ihsan Fadilah, Oxford University Clinical Research Unit Indonesia
execute:
  echo: true
  warning: false
autocomplete: false
...
We will explore some R code for a 1st wave of COVID-19 in
...
# 2026 Infectious Diseases Modelling
...
We'll be making use of the first wave data. Here is a reminder
of what the first wave looked like:
...
```{r load-data}
first_wave <- read.csv("first_wave_COVID.csv",
...
ggplot(first_wave, aes(x = Date, y = Cases)) +
    ...
    ylab("Daily cases") +
    xlab("") +
    ggtitle("COVID First Wave, Jan-Jun 2020") +
    theme_bw()
```
...
ggplot(outbreak, aes(x = Date, y = Cases)) +
    ...
    ylab("Daily cases") +
    xlab("") +
    ggtitle("Filtered data from the First Wave, with intervention timings") +
    theme_bw() +
    ...
    annotate(
        geom = "text",
        label = c("Emergency", "Emergency + Curfew"),
...
parameters <- c(
    ...
    curfew_efficacy = 0.80 # guess (note that this is efficacy due to the Emergency + Curfew)
)
```
...
# Initial conditions
Total_population <- 6.6e7 # Rough population estimate
...
negative.log.likelihood <- function(input_parameters,
    ...
    parms_base[c("emergency_efficacy", "curfew_efficacy")] <-
        input_parameters[c("emergency_efficacy", "curfew_efficacy")]
...
```{r using-optim}
optimal_efficacy_parameters <- optim(
    par = init_parameters,
    fn = negative.log.likelihood,
    method = "L-BFGS-B",
    lower   = c(emergency_efficacy = 0, curfew_efficacy = 0),
    upper   = c(emergency_efficacy = 1, curfew_efficacy = 1),
    hessian = TRUE
)

# Check for convergence
optimal_efficacy_parameters$convergence # 0 - converged; nonzero - failed

# Optimal parameters:
optimal_efficacy_parameters$par[c(
    "emergency_efficacy",
    "curfew_efficacy"
)]

# Inspect the optimal negative log-likelihood
optimal_efficacy_parameters$value
```
...
optimal_parameters[c("emergency_efficacy", "curfew_efficacy")] <- optimal_efficacy_parameters$par[c("emergency_efficacy","curfew_efficacy")]
...
covar_matrix <- solve(optimal_efficacy_parameters$hessian)
std_errors <- sqrt(diag(covar_matrix))
estimates <- data.frame(
    estimate = optimal_efficacy_parameters$par,
...
Now we have a model for the COVID-19 transmission, and we
...
scenario.exploration = function(emergency_eff, curfew_eff, emergency_start, curfew_start, plot_title){
  
  new_params = parameters 
  new_params[c("emergency_efficacy", "curfew_efficacy")] = c(emergency_eff, curfew_eff)
...
ggplot(outbreak) +
    ...
    geom_ribbon(
        data = optimal_solution[-1, ],
        aes(x = Date, ymin = lower50, ymax = upper50, fill = "solution"),
        colour = "firebrick2", alpha = 0.8
    ) +
    geom_ribbon(
        data = optimal_solution[-1, ],
        aes(x = Date, ymin = lower95, ymax = upper95, fill = "solution"),
        colour = "firebrick2", alpha = 0.5
    ) +
    geom_ribbon(
        data = observational_intervals[-1, ],
        aes(x = Date, ymin = lower50, ymax = upper50, fill = "counterfactual"),
        colour = "darkorchid4", alpha = 0.8
    ) +
    geom_ribbon(
        data = observational_intervals[-1, ],
        aes(x = Date, ymin = lower95, ymax = upper95, fill = "counterfactual"),
        colour = "darkorchid4", alpha = 0.5
    ) +
    geom_point(data=new_solution[-1,], aes(x=Date, y=Incidence), size=2, colour="darkorchid4") +
    labs(x = "", y = "Daily cases", title = plot_title, fill = "") +
    geom_vline(
        aes(xintercept = emergency_start),
        linetype = "dashed", colour = "firebrick"
    ) +
    geom_vline(
    ...
    annotate(
        geom = "text",
        label = c("Emergency", "Emergency + Curfew"),
        x = c(emergency_start, curfew_start),
        y = c(165, 165),
        angle = 90,
        vjust = 1.5
    ) +
    ...
    scale_fill_manual(
        values = c("darkorchid4", "firebrick2"), labels = c("Counterfactual", "Optimal Solution"))
}

### No intervention ###
scenario.exploration(emergency_eff=0, curfew_eff=0, 
                     emergency_start=emergency_start_date, 
...
scenario.exploration(emergency_eff=optimal_parameters["emergency_efficacy"], 
