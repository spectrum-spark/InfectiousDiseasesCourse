...
presenter: '<a href="https://research.monash.edu/en/persons/michael-lydeamore">Michael Lydeamore</a>, Department of Econometrics and Business Statistics, Monash University'
contributors:
  ...
  - 'With thanks to <a href="http://www.tropmedres.ac/wirichada-pan-ngum">Wirichada Pan-Ngum</a>, Faculty of Tropical Medicine, Mahidol University'
execute:
  echo: true
  warning: false
autocomplete: false
...
##################################
```
Open your course RStudio Project, create a new R script named `session9_interventions.R`, save it with your course files and copy the header into it. Install any missing packages once with `install.packages(c("dplyr", "deSolve", "ggplot2", "readr"))`.
...
We'll be making use of the first-wave Thailand data. [Download `first_wave_TH.csv`](../first_wave_TH.csv){download="first_wave_TH.csv"} and save it in the same directory as your R script. Here is a reminder of what the first wave in Thailand looked like:
...
We're going to model two interventions: the emergency declaration on 26 March and the start of curfew on 3 April.
...
[Download the code for this session so far](../session9_interventions_initial_model.R){download="session9_interventions_initial_model.R"}. Keep `first_wave_TH.csv` in the same directory before running it.
:::
...
::: {.callout-tip title="Need to catch up?"}
[Download the code for this session so far](../session9_interventions_calibrated_model.R){download="session9_interventions_calibrated_model.R"}. Keep `first_wave_TH.csv` in the same directory before running it.
:::
