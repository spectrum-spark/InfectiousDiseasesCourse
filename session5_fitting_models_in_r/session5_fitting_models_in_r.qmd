...
presenter: '<a href="https://github.com/EamonConway">Eamon Conway</a>, The Walter and Eliza Hall Institute, Melbourne'
contributors:
  - 'With thanks to <a href="http://www.tropmedres.ac/researchers/researcher/lisa-white">Lisa White</a>, Nuffield Department of Medicine, Oxford University'
  - 'With thanks to <a href="https://research.jcu.edu.au/portfolio/michael.meehan1/">Michael Meehan</a> and <a href="https://research.jcu.edu.au/portfolio/emma.mcbryde">Emma McBryde</a>, Australian Institute of Tropical Health & Medicine, James Cook University'
  - 'Adapted by the presenter, <a href="https://github.com/EamonConway">Eamon Conway</a>, The Walter and Eliza Hall Institute, Melbourne'
date: March 2023
...
# Model fitting
The first exercise in this session is to fit a model "by eye". We hope that by the end of the exercise you will be thinking to yourself "there must be an easier and more robust way to do this". This motivates the rest of the session. Open the [browser-based least-squares application](https://spectrum-spark.github.io/app-seir-least-squares/). The dots represent the data and the line represents predicted values based on a simple cubic model. Adjusting the model parameters changes the sum of squared residuals (SSQ), giving an intuitive feel for fitting by minimising SSQ.
...
To begin, open your course RStudio Project, create a new R script named `fitting_models.R`, and save it with your course files. Install any missing packages once with `install.packages(c("dplyr", "deSolve", "ggplot2", "purrr", "readr", "tidyr"))`, then copy the code below into your script.
...
Before continuing, [download `first_wave_TH.csv`](../first_wave_TH.csv){download="first_wave_TH.csv"} and save it in the same directory as your R script.
...
[Download the code for this session so far](../session5_fitting_models_least_squares.R){download="session5_fitting_models_least_squares.R"}. Keep `first_wave_TH.csv` in the same directory before running it.
:::
...
sir <- tibble::tibble(t = 0:110, y = exp(0.05 * t))
observation_times <- c(25, 50, 75, 100)

poisson_data <- sir |>
    filter(t %in% observation_times) |>
    mutate(observed = rpois(n(), lambda = y))

negative_binomial_data <- sir |>
    filter(t %in% observation_times) |>
    mutate(observed = rnbinom(n(), mu = y, size = 5))
...
distribution_data <- tidyr::expand_grid(
    ...
    left_join(sir, by = "t") |>
    mutate(
        poisson_width = dpois(height, lambda = y),
        negative_binomial_width = dnbinom(height, mu = y, size = 10)
    )

ggplot() +
    geom_ribbon(
        data = distribution_data |> filter(poisson_width >= 1e-10),
        aes(xmin = t, xmax = t - 90 * poisson_width, y = height, group = t, fill = "Model Fit", color = "Model Fit"),
        outline.type = "full", alpha = 0.4
    ) +
    geom_point(
        data = poisson_data,
        aes(x = t, y = observed, color = "Data", fill = "Data")
    ) +
...
ggplot() +
    geom_ribbon(
        data = distribution_data |> filter(negative_binomial_width >= 1e-10),
        aes(xmin = t, xmax = t - 90 * negative_binomial_width, y = height, group = t, fill = "Model Fit", color = "Model Fit"),
        outline.type = "full", alpha = 0.4
    ) +
    geom_point(
...
Recalculate the optimal model solution using the maximum-likelihood estimates so that the following plots and uncertainty calculations use the correct fit.

```{r mle-optimal-solution}
optimal_parameters <- parameters
optimal_parameters["R0"] <- optimum_parameters["R0"]
...
::: {.callout-tip title="Need to catch up?"}
[Download the code for this session so far](../session5_fitting_models_maximum_likelihood.R){download="session5_fitting_models_maximum_likelihood.R"}. Keep `first_wave_TH.csv` in the same directory before running it.
:::
...
::: {.callout-tip title="Need to catch up?"}
[Download the code for this session so far](../session5_fitting_models_complete.R){download="session5_fitting_models_complete.R"}. Keep `first_wave_TH.csv` in the same directory before running it. The optional grid search can take longer than the other sections.
:::
