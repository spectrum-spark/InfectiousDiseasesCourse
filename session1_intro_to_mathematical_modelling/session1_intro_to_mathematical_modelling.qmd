...
$\mathcal{R}_0$ is defined as the average number of new infections that result from a single infection introduced into a totally susceptible population. This will depend on:
...
* The duration of infection, $1/\gamma$

The equation for the basic reproduction number is
$$\mathcal{R}_0=\frac{pc}{\gamma}.$$
The product, $pc$, can be replaced by the single parameter, $\beta$, defined as the effective contact rate per unit time, giving
$$\mathcal{R}_0=\frac{\beta}{\gamma}.$$

## Estimating $\mathcal{R}_0$ from your simulation experiment

1. For $1/\gamma$, what were the value and units for the duration of infection?
<textarea name="Text1" cols="127" rows="1"></textarea>
...
4. What does that give us for $\beta=pc$?
<textarea name="Text1" cols="127" rows="4"></textarea>

5. Explain why the basic reproduction number is 1.9.
<textarea name="Text1" cols="127" rows="4"></textarea>
...
### Some theory
* The effective reproduction number $\mathcal{R}_e(t)$ is defined as the average number of new infections caused by an infected individual at any stage during the epidemic when there will be both susceptible and recovered (thus immune) individuals present
* $\mathcal{R}_e(t)=\mathcal{R}_0 x^∗$ varies over time depending on the fraction of the population susceptible, $x^∗=\left(\frac{S}{pop}\right)$
* Incidence of infections increases when $\mathcal{R}_e(t)>1$ and declines when $\mathcal{R}_e(t)<1$. Thus the **critical proportion susceptible**, $x^∗$, can be calculated for $\mathcal{R}_e(t)=1$:
$$\mathcal{R}_e(t)=1\Rightarrow \mathcal{R}_0x^∗=1 \Rightarrow x^∗=\frac{1}{\mathcal{R}_0}$$

## Implications for vaccination
* Critical proportion immune or the herd immunity threshold:
$$HIT = 1-x^* \Rightarrow HIT = 1 - \frac{1}{\mathcal{R}_0}$$
...
6. If the target coverage for a particular vaccine is 90%, what can you infer about the $\mathcal{R}_0$ of the pathogen in question?
