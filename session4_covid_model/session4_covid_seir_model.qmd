...
format:
    training-html:
      ...
      presenter: Dewi Anastasia Christina, Eijkman Molecular Biology Research Group (BRIN)/School of Public Health (Imperial)
      contributors:
...
Over the next three sessions, we will explore an example modelling process where an outbreak has occurred with set interventions. Our model aims to explore the impact of the interventions that were conducted and explore the counter-factuals (what might have happened if there were no interventions).
...
-   Model the interventions and update the model fit. Explore the impact of the interventions and the counter-factuals.
...
-   Code summary
-   Generate R project for this workshop
...
You must let R know the location of any files you plan to import, so remember to set your R project. Follow the instruction below to create a new R project:

:::: row
::: {style="text-align:center"}
<p>
1.  Open **RStudio**, navigate to **File**, then choose **New Project.**
</p>
<img src="picts/new_R_project1.png" style="width:80%"/>
:::
::::

:::: {#fig-project .text-center}
<p>
2. To create a project, click on **New Directory** and choose **New Project.**
</p>
<div style="display: flex; justify-content: center; gap: 10px;">
  <img src="picts/new_R_project2.png" style="width:40%;"/>
  <img src="picts/new_R_project3.png" style="width:40%;"/>
</div>
::::
...
::: {style="text-align:center"}
<p>
3.  Name your project, select a location, and click **Create Project**.
</p>
<img src="picts/new_R_project4.png" style="width:40%"/>
:::
::::
...
::: {style="text-align:center"}
<p>
4. The new project is now opened in its own session, with the working directory automatically set to the folder you specified.
</p>
<img src="picts/new_R_project5.png" style="width:60%"/>
:::
::::
<p>
At this stage, it is best to organize your project by creating folders for your data and R scripts. Place all datasets in the **data/** folder, and store your R scripts in the **R/** folder, using a separate script for each workshop session, as illustrated below:
</p>
:::: {#fig-project .text-center}
<div style="display: flex; justify-content: center; gap: 10px;">
  <img src="picts/new_R_project_data_management1.png" style="width:40%;"/>
  <img src="picts/new_R_project_data_management2.png" style="width:40%;"/>
</div>
::::
...
```{r load-data}
# Data imports
first_wave <- read.csv("data/first_wave_COVID.csv",
                       colClasses = c("Date", "numeric", "numeric"))
# Convert dates
first_wave$Date <- as.Date(first_wave$Date,
                           format = "%Y-%m-%d")
summary(first_wave)
...
ggplot(first_wave) +
  geom_col(aes(x=Date, y=Cases),
           width=1, fill="dodgerblue2", colour="blue") +
  ylab("Daily cases") +
  xlab("") +
...
out <- ode(y = state,
           ...
           parms = parameters)
# Plot solution
par(mar = c(2, 2, 1, 1)) # reduce the margins of the plot in order to fit it in the panel
plot(out,
     xlab = "Time (days)",
...
ggplot(data = tidied_output) +
  geom_line(aes(x = Date, y = Population),
            linewidth = 1,
            colour = "firebrick") +
  ggtitle("COVID19's First Wave, Jan-Jul 2020") +
...
# Incidence
ggplot(data=tidied_output) +
  geom_col(data = first_wave,
           aes(x=Date, y=Cases),
           width=1, fill=NA, colour="black") +
  geom_line(aes(x=Date, y=Incidence),
...
ggplot(tidied_output) +
  geom_point(aes(x=Date, y=Data_cum_cases),
             size=2, colour="skyblue") +
  geom_line(aes(x=Date, y=Cumulative_incidence),
            linewidth=1, colour="firebrick") +
...
long_out <- ode(y = state, times = as.numeric(times_long - times_long[1]),
                func = COVID.base, parms = parameters)
