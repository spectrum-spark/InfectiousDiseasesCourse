...
<!-- fixme: Start with dV/dt = a, including solving it in a spreadsheet to give one example of ODE numerical solve using euler to show what has been done by ode() -->
...
# 1. t : time, which is the independent variable in our case
...
solve the SIR model following the same steps described above for the population growth model. Plot your solution to generate the following figure:  <!-- fixme: students should need to complete something about the equations themselves.-->
...
2. The basic reproduction number $\mathcal{R}_0$ is defined as the average number of new cases generated by a typical infectious individual when introduced into a totally susceptible population. For the SIR model described above, the basic reproduction number is $\mathcal{R}_0 = \beta / \gamma$. Calculate the $\mathcal{R}_0$ for the parameter values given above.
<textarea cols="127" rows="2"></textarea>

3. By considering a range of values for the transmission rate $\beta$, describe how the epidemic trajectory varies as a function of $\mathcal{R}_0$. In particular, what happens when $\mathcal{R}_0<1$?
<textarea cols="127" rows="2"></textarea>
...
To complete the description of our model we supply initial conditions for the predator and prey populations which we denote $y_0$ and $x_0$ respectively. In particular we take for parameter values and initial conditions: $\alpha=1$, $\beta=2$, $\delta=1$ and $\gamma=1$; $y_0=0.5$ and $x_0=2$.
