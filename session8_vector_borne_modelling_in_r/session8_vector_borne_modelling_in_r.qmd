...
subtitle: "SPARK short course, Indonesia, March 2023"
presenter: '<a href="https://www.doherty.edu.au/people/patricia-trish-campbell">Trish Campbell</a>, The University of Melbourne at The Peter Doherty Institute for Infection and Immunity'
contributors:
  - 'Developed by the presenter, <a href="https://www.doherty.edu.au/people/patricia-trish-campbell">Trish Campbell</a>, The University of Melbourne at The Peter Doherty Institute for Infection and Immunity'
execute:
  echo: false
  warning: false
editor: 
...
session. Open your course RStudio Project and [download the Session 8 starter
script](../session8_vector_model.R){download="session8_vector_model.R"}. Save it
with your course files and open it in RStudio. The script already contains the
header below.
...
Save your script regularly as you work through the practical session.
...
`install.packages(c("dplyr", "deSolve", "ggplot2"))`. The starter script then
loads them with `library()`.
...
```{r load_packages_student, echo=TRUE, eval=FALSE}
# Library imports
library(dplyr)
library(deSolve)
library(ggplot2)
...
```{r load_parameters, echo=FALSE, eval=TRUE}
...
```{r load_parameters_student, echo=TRUE, eval=FALSE}
# Model parameters
parameters <- c(
...
```{r set_up_times, echo=FALSE, eval=TRUE}
...
```{r set_up_times_student, echo=TRUE, eval=FALSE}
# Time window
start_date = as.Date("2008-11-02")
end_date = as.Date("2009-05-31")
...
```{r load_initial, echo=FALSE, eval=TRUE}
...
```{r load_initial_student, echo=TRUE, eval=FALSE}
# Initial conditions
...
```{r states_student, echo=TRUE, eval=FALSE}
...
```{r model_function, echo=FALSE, eval=TRUE}
...
```{r model_function_student, echo=TRUE, eval=FALSE}
# Model function
dengue_base <- function(t, state, parameters){
  with(as.list(c(state, parameters)), {
    # Calculate the total population sizes
...
```{r run_model_student, echo=TRUE, eval=FALSE}
# Solve model
...
```{r new_infections, echo=TRUE, eval=TRUE}
...
[Download the code for this session so far](../session8_vector_model_part1.R){download="session8_vector_model_part1.R"}. This runnable script contains the completed Part 1 model.
:::
...
```{r load_parameters_with_E, echo=FALSE, eval=TRUE}
...
```{r load_parameters_with_E_student, echo=TRUE, eval=FALSE}
# Model parameters
parameters <- c(
 bites = 0.63, # number of bites per mosquito per day
 T_HM = 0.26, # probability of transmission - mosquito to human
...
```{r load_initial_with_E, echo=FALSE, eval=TRUE}
...
```{r load_initial_with_E_student, echo=TRUE, eval=FALSE}
# Initial conditions
human_population <- 150000 # population of Cairns in 2008
initial_human_susceptible <- 149998
initial_human_exposed <- 0
...
```{r states_with_E, echo=FALSE, eval=TRUE}
# This code chunk initialises the compartments but does not show the completed code to the 
# students. Students need to complete the chunk named 'states_student'
state <- c(Susceptible_human = initial_human_susceptible,
...
```{r states_with_E_student, echo=TRUE, eval=FALSE}
...
```{r set_up_times_with_E, echo=FALSE, eval=TRUE}
...
```{r set_up_times_with_E_student, echo=TRUE, eval=FALSE}
# Time window
start_date = as.Date("2008-11-02")
end_date = as.Date("2011-05-31")
times = seq(start_date, end_date, by = 1)
...
```{r model_E_function, echo=TRUE, eval=TRUE}
# Model function
...
```{r run_model_with_E_student, echo=TRUE, eval=FALSE}
# Solve model
...
```{r run_model_with_E, echo=FALSE, eval=TRUE}
...
```{r new_infections_with_E, echo=TRUE, eval=TRUE}
# Convert ODE output to a tibble
dengue_with_E_infections <- tibble::as_tibble(
...
::: {.callout-tip title="Need to catch up?"}
[Download the code for this session so far](../session8_vector_model_part2.R){download="session8_vector_model_part2.R"}. This runnable script contains the completed models from Parts 1 and 2.
:::
...
```{r part_2_solutions, echo=TRUE, eval=FALSE}
#############################################################
## MODELLING VECTOR-BORNE DISEASES PRACTICAL SESSION PART 1##
#############################################################
...
```{r add_seasonality, echo=TRUE, eval=TRUE}
...
```{r load_initial_with_E_seasonal, echo=TRUE, eval=TRUE}
# Initial conditions
human_population <- 150000 # population of Cairns in 2008
initial_human_susceptible <- 149998
initial_human_exposed <- 0
...
```{r set_up_times_with_E_seasonal, echo=FALSE, eval=TRUE}
# This code chunk sets up the times for which to solve the odes but does not show the completed code to the 
# students. Students need to complete the chunk named 'set_up_times_student'
start_date = as.Date("2008-11-02")
end_date = as.Date("2009-05-31")
...
```{r set_up_times_with_E_seasonal_student, echo=TRUE, eval=FALSE}
# Time window
start_date = as.Date("2008-11-02")
end_date = as.Date("2009-05-31")
times = seq(start_date, end_date, by = 1)
...
```{r model_E_season_function, echo=TRUE, eval=TRUE}
...
```{r run_model_with_E_seasonal_student, echo=TRUE, eval=FALSE}
# Solve model
...
```{r run_model_with_E_seasonal, echo=FALSE, eval=TRUE}
# This code chunk runs the model with E and plots the output but does not show the completed code to the 
# students. Students need to complete the chunk named 'run_model_with_E_student'
out_dengue_with_E_seasonal <- ode(y = state, times = as.numeric(times - times[1]), func = dengue_with_E_seasonal, parms = parameters)
...
```{r new_infections_with_E_seasonal_infectious, echo=TRUE, eval=TRUE}
# Convert ODE output to a tibble
...
::: {.callout-tip title="Need to catch up?"}
[Download the code for this session so far](../session8_vector_model_part3.R){download="session8_vector_model_part3.R"}. This runnable script contains the completed models from Parts 1, 2 and 3.
:::
...
```{r part_3_solutions, echo=TRUE, eval=FALSE}
#############################################################
## MODELLING VECTOR-BORNE DISEASES PRACTICAL SESSION PART 1##
#############################################################
