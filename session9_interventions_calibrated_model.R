# Session 9 cumulative checkpoint: calibrated intervention model
...
negative_log_likelihood <- function(transformed_parameters, # assumes logit form of parameters -- per Wednesday arvo session
                                    data = outbreak$Cases[-1],
                                    initial_state = state,
                                    model_times = times,
                                    model_function = covid_intervention, # note this is our ode solver
                                    model_parameters = parameters) {

    # Back-transform model parameters (inverse logit)
    model_parameters[c("emergency_efficacy", "curfew_efficacy")] <-
        inverse_logit(transformed_parameters[c("emergency_efficacy", "curfew_efficacy")])

    # Solve model with updated parameters
    out <- solve_intervention_model(
        initial_state,
        model_times,
        model_function,
        model_parameters
...
negative_log_likelihood(init_parameters)
...
c(inverse_logit(optim_NM$par[c(
    "emergency_efficacy",
    "curfew_efficacy"
)]))
...
optimal_solution <- solve_intervention_model(
    initial_state = state,
    model_times = times,
    model_function = covid_intervention,
...
estimates_transformed <- tibble::tibble(
...
estimates <- inverse_logit(estimates_transformed[, -2])
estimates
