# Session 5 cumulative checkpoint: complete session
...
nll_grid <- function(R0_vec, initial_infectious_vec) {
    tidyr::expand_grid(
        R0 = R0_vec,
        initial_infectious = initial_infectious_vec
    ) |>
        mutate(NLL = purrr::map2_dbl(
            R0,
            initial_infectious,
            ~ negative_log_likelihood(log(c(
                R0 = .x,
                Initial_infectious = .y
            )))
        ))
}

# call the function that will calculate likelihood surface
nll_grid_results <- nll_grid(R0_vec, initial_infectious_vec)
